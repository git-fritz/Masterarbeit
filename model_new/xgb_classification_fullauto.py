# -*- coding: utf-8 -*-
"""
Created on Mon Mar 24 14:03:48 2025
...
df_summary.to_csv(os.path.join(out_dir, f"{target}_xgb_metrics.csv"), index=False)
print(df_summary)
...
# Save threshold
with open(os.path.join(out_dir, f"{target}_optimal_threshold.json"), "w") as f:
...
results_df.to_csv(os.path.join(out_dir, f"{target}_thresholded_results.csv"), index=False)
