# -*- coding: utf-8 -*-
"""
Created on Thu Mar 27 11:15:19 2025
...
exclude_features = ['id', 'avg_width', 'max_width', 'width_hist', 'width_bins', 'UniqueID',
    ...
    'binary_recovery', 'geometry', 'plot_avg_aspect']
# ------------------------------------------------ #

# Logic for output ID
model_type = "xgb"
target_type = "segment" if target_folder.endswith("_100") else "plot"
dataset_type = "auto" if "/auto" in output_root.replace("\\", "/").lower() else "manual"
full_id = f"{model_type}_{target}_{target_type}_{dataset_type}"
...
}).to_csv(os.path.join(out_dir, f"{full_id}_metrics_summary.csv"), index=False)
...
thr_j, thr_g = thresholds[idx_j], thresholds[idx_g]
...
plt.ylabel("Score")
plt.title(f"Threshold Comparison for {full_id}")
plt.legend()
plt.tight_layout()
...
results_df = pd.DataFrame({
    ...
    "thresholded_misclassified": (y_vali != final_preds).astype(int)
})
...
plt.title(f"Top {top_n_features} Features: {full_id}")
plt.tight_layout()
...
plt.title(f"Misclassification: {full_id}")
plt.tight_layout()
...
results_geo.to_file(os.path.join(out_dir, f"{full_id}_error_map.gpkg"), driver="GPKG")
